...
def game(difficulty):
    ...
    while True:
        ...
        if(tickCounter % FPS == 0):
            config["time"] -= 1
        windowSurface.fill((149, 53, 83))

        if (amountOfEnemies == 0):
            config["time"] = STARTINGTIME
            while(amountOfEnemies != config.get("maxAmountOfEnemies")):
                enemies.append(pygame.Rect((random.randint(0,730 - config.get("enemySize"))),
                                           (random.randint(0,730 - config.get("enemySize"))),
        ...
        pygame.draw.line(windowSurface, WHITE, (mouseX + 15, mouseY),
                        (mouseX - 15, mouseY), 2)
        misses = totalShots - score
        drawText("Time: " + str(config.get("time")), windowSurface, 8,8)
        drawText("Hits: " + str(score), windowSurface, 8,38)
        drawText("Misses: " + str(misses), windowSurface, 8,68)
        pygame.display.update()
        mainClock.tick(FPS)
Menu()
